...
pub mod fcm_storage {
    ...
    #[doc = "FCMStorage was auto-generated with ethers-rs Abigen. More information at: https://github.com/gakonst/ethers-rs"]
    use std::sync::Arc;
    pub static FCMSTORAGE_ABI: ethers::contract::Lazy<ethers::core::abi::Abi> =
    ...
    pub static FCMSTORAGE_BYTECODE: ethers::contract::Lazy<ethers::core::types::Bytes> =
        ethers::contract::Lazy::new(|| {
            "" . parse () . expect ("invalid bytecode")
        });
    pub struct FCMStorage<M>(ethers::contract::Contract<M>);
    impl<M> Clone for FCMStorage<M> {
        fn clone(&self) -> Self {
            FCMStorage(self.0.clone())
        }
    }
    impl<M> std::ops::Deref for FCMStorage<M> {
        type Target = ethers::contract::Contract<M>;
        fn deref(&self) -> &Self::Target {
            &self.0
        }
    }
    impl<M: ethers::providers::Middleware> std::fmt::Debug for FCMStorage<M> {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_tuple(stringify!(FCMStorage))
                .field(&self.address())
                .finish()
        }
    }
    impl<M: ethers::providers::Middleware> FCMStorage<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), FCMSTORAGE_ABI.clone(), client).into()
        }
        ...
        > {
            let factory = ethers::contract::ContractFactory::new(
                FCMSTORAGE_ABI.clone(),
                FCMSTORAGE_BYTECODE.clone().into(),
                client,
            );
    ...
    impl<M: ethers::providers::Middleware> From<ethers::contract::Contract<M>> for FCMStorage<M> {
    ...
    pub enum FCMStorageCalls {
        Paused(PausedCall),
        Traders(TradersCall),
        UnderlyingToken(UnderlyingTokenCall),
    }
    impl ethers::core::abi::AbiDecode for FCMStorageCalls {
        fn decode(
            data: impl AsRef<[u8]>,
        ) -> ::std::result::Result<Self, ethers::core::abi::AbiError> {
            if let Ok(decoded) = <PausedCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(FCMStorageCalls::Paused(decoded));
            }
            if let Ok(decoded) =
                <TradersCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(FCMStorageCalls::Traders(decoded));
            }
            if let Ok(decoded) =
                <UnderlyingTokenCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(FCMStorageCalls::UnderlyingToken(decoded));
            }
            Err(ethers::core::abi::Error::InvalidData.into())
        }
    }
    impl ethers::core::abi::AbiEncode for FCMStorageCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                FCMStorageCalls::Paused(element) => element.encode(),
                FCMStorageCalls::Traders(element) => element.encode(),
                FCMStorageCalls::UnderlyingToken(element) => element.encode(),
            }
        }
    }
    impl ::std::fmt::Display for FCMStorageCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                FCMStorageCalls::Paused(element) => element.fmt(f),
                FCMStorageCalls::Traders(element) => element.fmt(f),
                FCMStorageCalls::UnderlyingToken(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<PausedCall> for FCMStorageCalls {
        fn from(var: PausedCall) -> Self {
            FCMStorageCalls::Paused(var)
        }
    }
    impl ::std::convert::From<TradersCall> for FCMStorageCalls {
        fn from(var: TradersCall) -> Self {
            FCMStorageCalls::Traders(var)
        }
    }
    impl ::std::convert::From<UnderlyingTokenCall> for FCMStorageCalls {
        fn from(var: UnderlyingTokenCall) -> Self {
            FCMStorageCalls::UnderlyingToken(var)
        }
    }
